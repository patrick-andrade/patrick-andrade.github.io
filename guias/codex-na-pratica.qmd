...
**Versão:** 0.2.1  
**Última revisão das fontes:** 14 de agosto de 2026  

- A leitura essencial cabe em cerca de 5 minutos; o restante é opcional.
- Exercícios, exemplos ou uso do terminal não estão presentes aqui.
  - O aprofundamento aponta para a documentação oficial da OpenAI.
  - Esse é um guia para o Codex, mas a lógica vale para Claude Code, Cursor, Antigravity, etc.
...
Este guia mostra que o salto para o Codex no aplicativo desktop é bastante simples.
...
**Por que / ganhos**
- Uma versão canônica no disco, sem o ciclo anexar / baixar / “qual é o   arquivo certo?”.
- O Explorador de Arquivos continua sendo a fonte da verdade.
- O que o chat registra fica sendo a tarefa, e não o acervo de arquivos.
...
- Separe entradas ("inputs"), referências (para consulta) e resultados ("outputs");
  - O usar o Codex você pode orientar quais subpastas importam.
...
- O escopo do trabalho fica humano e o receio de alterações indevidas diminui (além de você poder pedir para salvar o plano em um arquivo específico).

**Na prática**
- Peça um plano, verifique o entra e sai, quais arquivos mudam e como verificar os resultados.
- Confirme as decisões que julgar adequadas (ou revise pontos específicos) e só então peça a execução do plano já revisado.
...
**Recomendação.**
- Não deixe o acordo só no fio do chat.
  - Grave-o em arquivos na pasta do projeto.
- Isso é a prática de *spec-driven development* (desenvolvimento orientado por especificação)
  - A ideia é trabalhar com um acordo escrito (especificação) do que deve ser feito.
  - Essa é uma boa prática não só para código, mas é muito efetiva para pesquisa, análise e produção de documentos.

**Por que / ganhos**
- O disco vira memória além da janela de contexto:
  - Chats novos herdam o histórico sem colar tudo de novo, e você revisa um texto estável.
...
- Ao fechar uma fase, uma linha no changelog.md.  
...
**Recomendação.**
- Não deixe o modelo padrão o tempo todo (a tentação é sempre usar o modelo mais poderoso, mas ele nem sempre é necessário)
...
**Na prática.** Olhe o seletor de modelo e o esforço antes de enviar o pedido.
...
- Continue no mesmo chat só para corrigir a *mesma* meta
  - Se o projeto for muito grande pode ser interessante arquivar o que já concluiu ou não mais relevante
...
- Leia o pedido de permissão (comando, alvo, motivo) e não aprove no
  automático.
